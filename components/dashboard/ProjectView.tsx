 ...
 * recalcule depuis l'horloge (aucun serveur requis) et, pour les projets
 * stockés dans le navigateur, la fin du rendu est persistée en localStorage.
 */
export function ProjectView({
  ...
  useEffect(() => {
    setProject(initial);
    if (initial.status !== "rendering") return;

    const tick = () => {
      setProject((prev) => {
        const next = withRenderProgress(prev);
        if (persistLocal && next.status !== prev.status) {
  ...
  const remainingSeconds = Math.max(
    1,
    Math.ceil(((100 - project.renderProgress) / 100) * (RENDER_DURATION_MS / 1000))
  );
        ...
        {isRendering && (
          <div className="mt-5">
            ...
            <p className="mt-2 text-xs text-warm-gray" role="status">
              Rendu 4K en cours — temps restant estimé : {remainingSeconds} s.
              Vous pouvez quitter cette page, le rendu continue.
            </p>
          </div>
        )}
      </section>
